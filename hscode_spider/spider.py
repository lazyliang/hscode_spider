...
from hscode_spider.row import Hscode, BaseInfo, TaxInfo
...
def url2html(url, proxy):
    ...
    response = requests.get(url_link, timeout=3)
...
def parse_charter_labels(_code, details_div):
    """
        解析章节层级内容
    """
    charter_labels = details_div[7].table.tbody.find_all('td', class_='td-label')
    charters = []
    for charter_label in charter_labels:
        txt = charter_label.text
        txt = txt.replace('[?]', '')
        txt = txt.replace(' ', '')
        charters.append(txt)
    return charters
...
def parse_charter_txts(_code, details_div):
    """
         解析章节内容
    """
    charter_txts = details_div[7].table.tbody.find_all('td', class_='td-txt')
    charters_txts = []
    for charter_txt in charter_txts:
        txt = charter_txt.text
        txt = txt.replace('[?]', '')
        txt = txt.replace(' ', '')
        charters_txts.append(txt)
    return charters_txts
...
def parse_details(code, proxy=None):
    ...
    charter_label = parse_charter_labels(code, details)
    charter_txt = parse_charter_txts(code, details)
    return Hscode(base_info, tax_info, declarations, supervisions, quarantines, ciq_codes, charter_label, charter_txt)
...
def search_chapter(chapter, include_outdated=False, quiet=False, proxy=None):
    ...
    if not quiet:
        print('Item (with searching "' + chapter + '"' + (
            ' including outdated' if include_outdated else '') + ')' + ' num: ' + str(len(all_code)))
