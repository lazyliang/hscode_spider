...
class Hscode():
    ...
    def __init__(self, base_info, tax, declarations, supervisions, quarantines, ciq, charter_label, charter_txt):
        """
            构造函数
        """
        # 商品编码
        ...
        self.ciq_code = ciq
        # 所属章节label
        self.charter_label = charter_label
        # 所属章节txt
        self.charter_txt = charter_txt
    ...
    def __str__(self):
        ...
        if self.charter_label:
            arr_str = arr2json(self.charter_label)
            result += ', "charter_label": ' + arr_str
        if self.charter_txt:
            arr_str = arr2json(self.charter_txt)
            result += ', "charter_txt": ' + arr_str
        return result + ' }'
